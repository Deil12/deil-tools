...
// Inlined Firework_stage.js dependency: Ticker.js
...
const Firework_stage = (function StageFactory(window, document, Ticker) {
	'use strict';
  
  // Track touch times to prevent redundant mouse events.
	let lastTouchTimestamp = 0;

	// Firework_stage constructor (canvas can be a dom node, or an id string)
	...
	// track all Firework_stage instances
	Stage.stages = [];

	// allow turning off high DPI support for perf reasons (enabled by default)
	// Note: MUST be set before Firework_stage construction.
