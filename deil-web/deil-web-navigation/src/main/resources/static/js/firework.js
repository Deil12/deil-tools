...
// Firework_stage.disableHighDPI = true;
const trailsStage = new Firework_stage('trails-canvas');
const mainStage = new Firework_stage('main-canvas');
const stages = [
	trailsStage,
	mainStage
];
...
class Shell {
	...
	launch(position, launchHeight) {
		...
		comet.heavy = true;
		// comet spark trail
		comet.spinRadius = Firework_math.random(0.32, 0.85);
		...
		if (typeof this.color === 'string') {
			...
			if (this.ring) {
				...
				createParticleArc(0, PI_2, this.starCount, 0, angle => {
					...
					// Rotate ring
					const newSpeed = Firework_math.pointDist(0, 0, initSpeedX, initSpeedY);
					const newAngle = Firework_math.pointAngle(0, 0, initSpeedX, initSpeedY) + ringStartAngle;
					const star = Star.add(
						x,
						y,
						color,
...
const soundManager = {
	...
	playSound(type, scale=1) {
		...
		scale = Firework_math.clamp(scale, 0, 1);
		...
		const initialPlaybackRate = Firework_math.random(
		...
		const buffer = Firework_math.randomChoice(source.buffers);
