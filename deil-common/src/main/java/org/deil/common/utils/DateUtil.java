...
public class DateUtil {
	...
	/**
	 * @date 2022/7/7, 下午12:22 : 获取指定 String
	 * @param pattern
	 * @return java.lang.String
	 */
	public static String getFormatStr(String pattern) {
		String dateStr = new SimpleDateFormat(pattern).format(new Date());
		return dateStr;
	}

	/**
	 * @date 2022/7/7, 下午12:57 : todo 获取当地指定 String
	 * @param pattern
	 * @return java.lang.String
	 */
	public static String getFormatStrForLocation(String pattern) {
		return toDateTimeStr(LocalDateTime.now(), pattern);
	}

	/*public static Date getFormatDateForLocation(String pattern) {
		return toDateTimeStr(LocalDateTime.now(), pattern);
	}*/

	public static String toDateTimeNum(LocalDateTime date) {
		return toDateTimeStr(date, YYYYMMDDHHMMSS);
	}
	...
	public static List<String> getSpecifyDaysOfMonthInString(Date begin, Date end, DateFormat dateFormat) {
		...
		for (Date date : tmplist) {
			rs.add(Optional.ofNullable(df).orElse(new SimpleDateFormat("yyyy-MM-dd")).format(date));
		}
		return rs;
	}
	...
	* @CreateTime:15:32
	* @Author:
	* @Description:字符串格式转成Date日期格式（包含UTC日期格式）
	*/
	public static final Date strToDate(String dateStr) {
		Date toDate = new DateTime(dateStr).toDate();
		return toDate;
	}
	 ...
	 * @version 1.0
	 */
	public static final Timestamp strToTimestamp(String dateStr) {
		Date toDate = new DateTime(dateStr).toDate();
	...
	/**
	 * @date 2022/8/18, 下午3:42 : 字符串指定格式转成Date日期格式（包含UTC日期格式）
	 * @param dateStr
	 * @param format
	...
	public static final Date strToDate(String dateStr, String format) {
		Date toDate = null;
		try {
			toDate = new DateTime(new SimpleDateFormat(format)
